...
class TestValidateRequestPayload(unittest.TestCase):
    ...
    def test_invalid_check_item_request(self):
        with self.client:
            response = self.client.post('/check-item', data=json.dumps({'is_checked': 'not_a_boolean'}), content_type='application/json')
            self.assertEqual(response.get_json(), OutcomeResponse(success=False).model_dump_json())

    def test_valid_check_item_request(self):
        with self.client:
            response = self.client.post('/check-item', data=json.dumps({'is_checked': True}), content_type='application/json')
            self.assertEqual(response.get_json(), OutcomeResponse(success=True).model_dump_json())

    def test_invalid_delete_item_request(self):
        with self.client:
            response = self.client.post('/delete-item', data=json.dumps({'item_id': 123}), content_type='application/json')
            self.assertEqual(response.get_json(), OutcomeResponse(success=False).model_dump_json())

    def test_valid_delete_item_request(self):
        with self.client:
            response = self.client.post('/delete-item', data=json.dumps({'item_id': 'item123'}), content_type='application/json')
            self.assertEqual(response.get_json(), OutcomeResponse(success=True).model_dump_json())
    
    def test_invalid_add_item_request(self):
        with self.client:
            response = self.client.post('/add-item', data=json.dumps({'item_id': 'item123', 'message': 123}), content_type='application/json')
            self.assertEqual(response.get_json(), OutcomeResponse(success=False).model_dump_json())

    def test_valid_add_item_request(self):
        with self.client:
            response = self.client.post('/add-item', data=json.dumps({'item_id': 'item123', 'message': 'Added item'}), content_type='application/json')
            self.assertEqual(response.get_json(), OutcomeResponse(success=True).model_dump_json())

    def test_invalid_fetch_items_request(self):
        with self.client:
            response = self.client.post('/fetch-items', data=json.dumps({'user_identifier': 'user123'}), content_type='application/json')
            self.assertEqual(response.get_json(), OutcomeResponse(success=False).model_dump_json())

    def test_valid_fetch_items_request(self):
        # Simulate valid JSON payload for '/fetch-items' endpoint
        with self.client:
            response = self.client.post('/fetch-items', data=json.dumps({'user_identifier': 123}), content_type='application/json')
